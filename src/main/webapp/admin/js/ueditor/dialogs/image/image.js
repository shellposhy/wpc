/**
 * Created by JetBrains PhpStorm. User: taoqili Date: 12-01-08 Time: 下午2:52 To
 * change this template use File | Settings | File Templates.
 */
var imageUploader = {}, flashObj = null, postConfig = [];
(function() {
	var g = $G, ajax = parent.baidu.editor.ajax, maskIframe = g("maskIframe"); // tab遮罩层,用来解决flash和其他dom元素的z-index层级不一致问题
	// flashObj; //flash上传对象
	...
	function searchImage() {
		...
		g("searchList").innerHTML = "<p class='msg'>" + lang.imageLoading
				+ "</p>";
		var key = imgSearchInput.value, type = $G("imgType").value, url = "http://image.baidu.com/i?ct=201326592&cl=2&lm=-1&st=-1&tn=baiduimagejson&istype=2&rn=32&fm=index&pv=&word="
				+ encodeToGb2312(key) + type + "&" + +new Date;
		var reqCallBack = function(data) {
			...
			if (imgObjs.length < 2) {
				g("searchList").innerHTML = "<p class='msg'>" + lang.tryAgain
						+ "</p>";
				return;
			}
			for ( var i = 0, len = imgObjs.length; i < len - 1; i++) {
				var img = document.createElement("img"), obj = imgObjs[i], div = document
						.createElement("div");
				...
				img.setAttribute("title", lang.toggleSelect + obj.width + "X"
						+ obj.height);
				...
				p.innerHTML = "<a target='_blank' href='" + obj.fromURL + "'>"
						+ title + "</a>";
				div.appendChild(p);
				// setTimeout(function(){
				frg.appendChild(div);
	...
	function addScrollListener() {
		...
		g("imageList").onscroll = function() {
			var imgs = this.getElementsByTagName("img"), top = Math
					.ceil(this.scrollTop / 100) - 1;
			top = top < 0 ? 0 : top;
	...
	function insertSingle() {
		var url = g("url"), width = g("width"), height = g("height"), border = g("border"), vhSpace = g("vhSpace"), title = g("title"), align = findFocus(
				"remoteFloat", "name"), imgObj = {};
		if (!url.value)
			return;
		if (!flagImg)
		...
		imgObj.style = "width:" + width.value + "px;height:" + height.value
				+ "px;";
		insertImage(imgObj);
	...
	function createPreviewImage(url) {
		...
		if (!imgTypeReg.test(url) || url.indexOf(urlFilter) == -1) {
			preview.innerHTML = "<span style='color: red'>"
					+ lang.imageUrlError + "</span>";
			flagImg = null;
			return;
		}
	...
	function showImageInfo(img) {
		...
		g("url").value = wordImgFlag ? wordImgFlag.replace("&amp;", "&") : (img
				.getAttribute('_src') || img.getAttribute("src", 2).replace(
				"&amp;", "&"));
	...
	function showPreviewImage(img, needClone) {
		...
		var maxWidth = 262, maxHeight = 262, target = scaling(tmpWidth,
				tmpHeight, maxWidth, maxHeight);
		...
		preview.innerHTML = '<img src="' + target.src + '" width="'
				+ target.width + '" height="' + target.height + '" border="'
				+ target.border + 'px solid #000" />';
	}
	...
	function scaling(width, height, maxWidth, maxHeight) {
		...
		var srcRatio = (width / height).toFixed(2), tarRatio = (maxWidth / maxHeight)
				.toFixed(2), w, h;
	...
	function clickHandler(tabHeads, tabBodys, obj) {
		...
		for ( var j = 0, length = tabBodys.length; j < length; j++) {
			...
			if (id != tabSrc) {
								...
								.request(
										editor.options.imageManagerUrl,
										{
											...
											onsuccess : function(xhr) {
												// 去除空格
												var tmp = utils
														.trim(xhr.responseText), imageUrls = !tmp ? []
														: tmp
																.split("ue_separate_ue"), length = imageUrls.length;
												g("imageList").innerHTML = !length ? "&nbsp;&nbsp;"
														+ lang.noUploadImage
														: "";
				...
				var listOther = g("otherList");
				listOther.style.display = "none";
				// 切换到图片管理时，ajax请求后台图片列表
				if (id == "imgOther") {
					listOther.style.display = "";
					ajax.request("/pdcms/admin/imglib/album",{
						timeout : 100000,
						action : "post",
						onsuccess : function(data) {
							var albumList = eval(data.responseText);
							var albumId = 0;
							var obj = document.getElementById("albumList");
							obj.options.length = 0;
							if (albumList != null) {
								for ( var i = 0; i < albumList.length; i++) {
									var option = "";
									if (i == 0) {
										obj.options[obj.length] = new Option(albumList[i].name,albumList[i].id);
							...
							changeHandler();
							// 加载相册成功后，检索相册
							if (!listOther.children.length) {
								ajax.request("/admin/js/ueditor/jsp/imageOther.jsp?baseId="+ albumId+ "&uuid=",{
													timeout : 100000,
													action : "get",
													onsuccess : function(xhr) {
														var tmp = utils
													...
													onerror : function() {
														g("otherList").innerHTML = lang.imageLoadError;
													}
												});
							}
							// 加载相册图片结束
					...
					// alert(albumId);
					// alert(editor.options.imageOtherUrl);
					// 已经初始化过时不再重复提交请求
				}
				if (id == "imgSearch") {
	 ...
	 * 编辑缩略图事件
	 */
	function editThumbHandler() {
		var editBtn = document.getElementById("editThumb");
		editBtn.onclick = function() {
			// 比如 /pdcms
			var baseUrl = editor.options.appPathUrl;
			// 比如 /pdcms/admin/pic/745/4/list
			// 新建数据时，tableId和dataId不存在
			// 要对当前url进行验证
			// 比如当前url 是/pdcms/admin/pic///list
			var curUrl = editor.options.imageEditUrl;
			var buSize = baseUrl.length;
			var cuSize = curUrl.length;
			var size = cuSize - buSize;
			if (size > 17) {
				window.open(editor.options.imageEditUrl);
			} else {
				alert("新添加数据时，数据保存之前无法编辑文章缩略图！");
			}
			// window.location.href = editor.options.imageEditUrl;
	 ...
	 * 选择缩略图change事件
	 */
	function changeThumbHandler() {
		var thumbs = document.getElementById("curAlbumList");
		var editLabel = document.getElementById("editLabel");
		// 绑定事件
		thumbs.onchange = function() {
			var values = thumbs.value;
			if (values == 2) {
			...
			ajax
					.request(
							editor.options.imageManagerUrl + "&type=" + values,
							{
								timeout : 100000,
								action : "get",
								onsuccess : function(xhr) {
									// 去除空格
									var tmp = utils.trim(xhr.responseText), imageUrls = !tmp ? []
											: tmp.split("ue_separate_ue"), length = imageUrls.length;
									g("imageList").innerHTML = !length ? "&nbsp;&nbsp;"
											+ lang.noUploadImage
											: "";
									for ( var k = 0, ci; ci = imageUrls[k++];) {
										...
										img.onload = function() {
											...
											this.title = lang.toggleSelect + w
													+ "X" + h;
											this.onload = null;
										};
										img
	...
	function changeHandler() {
		var albums = document.getElementById("albumList");
		// 绑定事件
		albums.onchange = function() {
			// 执行检索
			ajax.request("/admin/js/ueditor/jsp/imageOther.jsp?baseId="+ albums.value + "&uuid=",
							{
								timeout : 100000,
								action : "get",
								onsuccess : function(xhr) {
